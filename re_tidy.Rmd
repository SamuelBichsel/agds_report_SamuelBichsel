...
This Markdown file is for the report exercise entitled *Cleaning data from elevated CO2 experiments* which is part of Chapter 3 (Data wrangling) of the AGDS Course. The goal is turning freely available, nice looking but untidy excel data from a scientific paper into a tidy dataset. 
...
Load the packages that are needed (tidyverse and dplyr), and then read the csv file (note: read_delim had to be used instead of read_csv because excel decided to make the csv with semicolons instead of commas for some obscure reason). 

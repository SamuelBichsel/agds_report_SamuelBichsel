...
#make the data wrangling part be a function so I can apply it to both datasets without using way too much code 
...
*Table 1: summary of 60-fold cross validation for tuning parameter k on the Davos training dataset*
```{r}
mod_Dav

```

*Table 2: summary of 60-fold cross validation for tuning parameter k on the Laegern training dataset*
```{r}
mod_Lae

```
Using MAE as a metric to tune the k, the best values are k = 25 for both the models of Davos and Laegern. 
...
knitr::kable(daily_fluxes_Dav_test |> 
    yardstick::metrics(GPP_NT_VUT_REF, Dav_fitted), caption = "*Table 3: Metrics of knn model fit vs. GPP measurements using the Davos training set to predict the Davos test set*")
...
knitr::kable(daily_fluxes_Lae_test |> 
    yardstick::metrics(GPP_NT_VUT_REF, Dav_fitted), caption = "*Table 4: Metrics of knn model fit vs. GPP measurements using the Davos training set to predict the Laegern test set*")

```


#### Laegern training, Laegern testing set
...
knitr::kable(daily_fluxes_Lae_test |> 
    yardstick::metrics(GPP_NT_VUT_REF, Lae_fitted), caption = "*Table 5: Metrics of knn model fit vs. GPP measurements using the Laegern training set to predict the Laegern test set*")
...
knitr::kable(daily_fluxes_Dav_test |> 
    yardstick::metrics(GPP_NT_VUT_REF, Lae_fitted), caption = "*Table 6: Metrics of knn model fit vs. GPP measurements using the Davos training set to predict the Davos test set*")


```


The results for these across-site model predictions aren't too bad, but mostly below the within-site, with all 3 metrics being slightly worse. Interestingly, the Laegern training set makes very similar predictions on both the Davos and Laegern testing sets. Otherwise, for both testing sites the RMSE is about 1 bigger and the R-Squared about 0.15-0.2 smaller (both worse) when comparing across-site and within-site predictions. Also, the Laegern training dataset performs worse on both training and testing than the Davos one, which could be due to, i.e., more outliers or irregularities/phases of higher GPP, or simply due to the smaller dataset making the training harder or less accurate. 
